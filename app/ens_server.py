...
def get_data():
    form = UploadForm()
    if form.validate_on_submit():
        data = pd.read_csv(form.features_file.data, index_col=0, float_precision='round_trip')
        target_name = form.target_name.data
        if form.target_file.data is not None:
            target = pd.read_csv(form.target_file.data, index_col=0, float_precision='round_trip')
            if target_name == '':
...
def model_page(name):
    ...
    learn_form.train_data.choices = [d for d in datasets if datasets[d].has_target]
