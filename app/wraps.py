...
class Ensemble:
    ...
    def fit(self, data_train, data_val=None):
        ...
        if data_val is not None:
            ...
            self.train_loss = self.model.fit(X_train, y_train, return_train_loss=True)[0]
    ...
    def predict(self, data_test):
        ...
        return pd.DataFrame(y_pred, index=data_test.data.index, columns=[self.target_name])
    ...
    def plot(self, loss_type='train'):
        ...
        ax.plot(np.arange(1, lim + 1), self.train_loss, label='train', c='b')
        if self.val_loss is not None:
            ax.plot(np.arange(1, lim + 1), self.val_loss, label='validation', c='m')
            varg = self.val_loss.argmin()
            ax.scatter(varg + 1, self.val_loss[varg], c='r', zorder=3, label='optimum')
